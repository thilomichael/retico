...
from retico.core.text.asr import ASRtoTTSModule
...
def simulation3(caller_dm, callee_dm, convtype):
...
def simulation2(caller_dm, callee_dm, convtype):
    caller_nlg = SimulatedNLGModule("data/%s/audio" % convtype, agent_type="caller")
...
def simulation(caller_dm, callee_dm, convtype):
    caller_nlg = SimulatedNLGModule("data/%s/audio" % convtype, agent_type="caller")
...
def convsim_simulation(convtype, simtype):
    ...
    if simtype == "gtts":
        simulation2(caller_dm, callee_dm, convtype)
    elif simtype == "mtts":
        simulation3(caller_dm, callee_dm, convtype)
    else:
        simulation(caller_dm, callee_dm, convtype)

def agenda_simulation(convtype, simtype):
...
def ngram_simulation(convtype, simtype):
...
def rasa_simulation(convtype, simtype):
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == "audio":
        ...
        convsim_simulation(sys.argv[2], simtype)
    elif sys.argv[1] == "agenda":
        agenda_simulation(sys.argv[2], simtype)
    elif sys.argv[1] == "ngram":
        ngram_simulation(sys.argv[2], simtype)
    elif sys.argv[1] == "rasa":
        rasa_simulation(sys.argv[2], simtype)
